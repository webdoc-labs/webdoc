  ...
  await Promise.all([
    ...
    (() => {
      ...
      for (const file of stylesheets) {
        ...
        const output = path.join(outDir, "imported/styles", path.parse(input).base);

        resolved.push(path.relative(outDir, output));
...
async function outSource(
  ...
  if (!source || !config.template.sources || config.template.repository) return;
